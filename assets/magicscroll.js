...
window.MagicScroll = (function() {
    ...
    b.prototype = {
        ...
        _setProperties: function() {
            ...
            if (v.browser.ieMode && v.browser.ieMode < 10) {
                ...
                this.container.jSetCssProp(g, "translate3d(0, -190px, 0)")
            }
        },
        _render: function() {
    ...
    v.extend(z.prototype, {
        ...
        renderCarousel: function(N) {
            ...
            v.defined(N) || (N = 0);
            this.lastAngle = N;
            let customClasses = ["prev-1", "prev-2", "next-1", "next-2", "active"];
            this.items.forEach((elem, index)=>elem.node.classList.remove(...customClasses));
            for (var L = 0; L < S; L++) {
                J = H = L * G + N;
                H %= this.circle;
                J %= this.circle;
                ...
                if (v.browser.ieMode && v.browser.ieMode < 10) {
                    ...
                    Q > 0 && (Q *= (-1));
                    // let translateY = I.top / ( Math.PI *  4);
                    // let translateZ = Q / (Math.PI * 6);
                    //I[this.p_.pos] = (Math.sin(H) * K[this.p_.pos] + parseInt(this.containerSize[this.p_.size]) / 2 - this.items[L].size[this.p_.size] / 2);
                    I[this.p_.pos] = (Math.sin(H) * K[this.p_.pos] + parseInt(this.containerSize[this.p_.size]) - this.items[L].size[this.p_.size] / 4 );
                    this.items[L].node.jSetCssProp(g, "translateX(" + I.left + "px)translateY(" + I.top + "px)translateZ(" + Q + "px)rotateX(0deg) rotateY(0deg) scale(1)")
                    //this.items[L].node.jSetCssProp(g, "translate3d("+ I.left +"px, " + I.top / 6 + "px, " + Q * 3 + "px) rotateX(0deg) rotateY(0deg) scale(1)")
                    //this.items[L].node.jSetCssProp(g, "translate3d("+ I.left +"px, " + I.top / ( Math.PI *  3.5) + "px, " + Q / (Math.PI * 4)  + "px) rotateX(0deg) rotateY(0deg) scale(1)")
                    //this.items[L].node.jSetCssProp(g, "translate3d(" + L.left + "px, " + L.top + "px, " + I.translateZ + "px)rotateX(" + G.top + "deg)rotateY(" + G.left + "deg)");
                }
                this.items[L].node.jSetCssProp("z-index", 0 + J);
                this.items[L].node.jSetOpacity(O);
                //console.log(J);
          //       let currentSlideZIndex = parseInt(this.items[L].node.jGetCss("z-index"));
          //       let activeIndex = -1;
          //       this.items[L].node.jSetCssProp("z-index", 0 + J);
          //       // this.items[L].node.classList.remove(...customClasses);
          //       if(currentSlideZIndex === 100){
          //         let len = this.items.length;
          //         let current = this.items[L].node.classList.add("active");
          //         let previous = this.items[(L+len-1)%len].node.classList.add("prev-1");
          //         let previousTransform = this.items[(L+len-1)%len].node.jGetCss("transform");
                  
          //         console.log(previousTransform);
          //         // translate3d(0px, -34px, -60px)
          //         let next = this.items[(L+1)%len].node.classList.add("next-1");
          //         let previous2 = this.items[(L+len-2)%len].node.classList.add("prev-2");
          //         let next2 = this.items[(L+2)%len].node.classList.add("next-2");
          //       }
          //       // else{
          //       //   this.items[L].node.classList.remove(...customClasses);
          //       // }
          ...
          //       let slideOpacity = parseInt(this.items[L].node.jGetCss("opacity"));
          //       if(slideOpacity === 0){
          //         this.items[L].node.style.display = "none";
          //       }else{
          //         this.items[L].node.style.display = "block";
          //       }
                

              
          //       // console.log(this.items[L].node.jGetCss("z-index"))
          //       // console.dir(this.items[L])
          //   }

          // if(document.querySelector(".prev-1")) {
          //   document.querySelector(".prev-1").style.transform = `translate3d(${I.left}px, ${-34}px, ${-50}px) rotateX(0deg) rotateY(0deg) scale(1)`;
          // }

          // if(document.querySelector(".prev-2")) {
          //   // px, -175px, -220px
          //   document.querySelector(".prev-2").style.transform = `translate3d(${I.left}px, ${-147}px, ${-220}px) rotateX(0deg) rotateY(0deg) scale(1)`;
          // }
          
          // if(document.querySelector(".next-1")) {
          //   document.querySelector(".next-1").style.transform = `translate3d(${I.left}px, 110px, -50px) rotateX(0deg) rotateY(0deg) scale(1)`;
          // }

          // if(document.querySelector(".next-2")) {
          //   // px, -175px, -220px
          //   document.querySelector(".next-2").style.transform = `translate3d(${I.left}px, 185px, -220px) rotateX(0deg) rotateY(0deg) scale(1)`;
           }
        },
        round: function(G, H) {
